...
implementation is:
\[
    x_{k+1} = x_k + H_{\vib}^{-1}\,\tilde{F}_{\mathrm{GAD}}(x_k),
\]
i.e., the same pseudoinverse step as NR but applied to the modified gradient.
...
\begin{lstlisting}[caption={Same step applied to the GAD modified gradient
...
After computing the vibrational pseudoinverse step $\delta x$, we scale it
to a \emph{current trust radius} $\Delta$ and evaluate whether the actual
energy change matches the quadratic model's prediction:
\[
...
\[
    \Delta_{k+1} =
    \begin{cases}
        \min(1.5\,\Delta_k,\;\madcap) & \text{if } \rho > 0.75 \quad\text{(excellent model)} \\
        0.5\,\Delta_k & \text{if } 0 \le \rho < 0.25 \quad\text{(poor model)} \\
        0.25\,\Delta_k & \text{if } \rho < 0 \quad\text{(very poor model)} \\
        0.25\,\Delta_k & \text{if step rejected} \quad(\delta E_{\mathrm{actual}} > 0) \\
        \Delta_k & \text{otherwise}
    \end{cases}
\]
Steps are \emph{rejected} and retried at a smaller radius (up to 10 retries) when the energy
increases (for minimization).

\subsection{Why $\rho > 0.75$ / $\rho < 0.25$ thresholds?}

These are the standard dogleg/trust-region thresholds from Nocedal \& Wright
\cite{nocedal2006numerical}: $\rho > 0.75$ means the local quadratic model
predicts at least 75\% of the actual improvement, so we trust it enough to
expand $\Delta$; $\rho < 0.25$ means the model is unreliable, so we contract.

Crucially, our experiments showed that standard contraction ($0.5\Delta_k$) is not aggressive enough when the model is completely wrong. We introduced two harsh penalties:
\begin{itemize}[noitemsep]
    \item If $\rho < 0.0$ (model predicted energy decrease, but it increased, or vice versa), we shrink the radius sharply to $0.25\Delta_k$.
    \item If the step is rejected entirely ($\delta E_{\mathrm{actual}} > 0$), we also shrink sharply to $0.25\Delta_k$ and retry up to 10 times.
\end{itemize}
This rapid collapse prevents the optimizer from burning through retries while repeatedly crashing into narrow canyon walls.

\begin{algorithm}
\caption{Trust Region step (NR minimization / GAD)}
\begin{algorithmic}[1]
...
\begin{enumerate}[noitemsep]
    \item \textbf{Adapt NR Trust Region into GAD}: Implement the exact dogleg/trust-region energy prediction logic ($\rho$ ratio, $1.3$\,\AA\ cap, harsh $0.25\times$ penalties) and the aggressive $8\times10^{-3}$ eigenvalue filtering into the GAD script to push its success rate up to 100\% with fewer steps.
