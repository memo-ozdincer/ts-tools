"""HIP Sella Hyperparameter Optimization v2 - Full dataset with eigenvalue-based early stopping.

This version:
- Runs on ALL samples (no difficult-only filtering)
- Uses 8000 max steps per sample
- Prunes individual samples if negative eigenvalue count doesn't decrease in 500 steps
- Uses narrowed hyperparameter ranges based on hip_sella_parallel_2.db analysis
"""
...
# Eigenvalue-based early stopping parameters
EIGVAL_PATIENCE = 500  # Stop if neg eigenvalues don't decrease in this many steps
EIGVAL_CHECK_INTERVAL = 50  # Check eigenvalues every N steps
...
class HPOResult:
    ...
    n_both: int = 0
    n_sella_only: int = 0
    n_early_stopped: int = 0  # Samples stopped due to eigenvalue stagnation
    ...
    def early_stop_rate(self) -> float:
        return self.n_early_stopped / self.n_samples if self.n_samples > 0 else 0.0
...
def run_sella_with_eigval_pruning(
    calculator,
    calculator_type: str,
    ...
    max_steps: int,
    device: str,
    eigval_patience: int = EIGVAL_PATIENCE,
    eigval_check_interval: int = EIGVAL_CHECK_INTERVAL,
    verbose: bool = False,
) -> Dict[str, Any]:
    """Run Sella TS optimization with eigenvalue-based early stopping.

    Stops optimization early if the number of negative eigenvalues hasn't
    decreased in `eigval_patience` steps.

    Returns:
        Dictionary with optimization results including early_stopped flag.
    """
    if not SELLA_AVAILABLE:
        raise ImportError("Sella is not installed")
    ...
    positions = coords0.detach().cpu().numpy().reshape(-1, 3)
    numbers = atomic_nums.detach().cpu().numpy().flatten().astype(int)
    atoms = Atoms(numbers=numbers, positions=positions)

    # Create ASE calculator
    ase_calc = create_ase_calculator(calculator, calculator_type, device=device)
    atoms.calc = ase_calc

    # Create hessian function for exact Hessians
    hessian_fn = None
    if config.use_exact_hessian:
        hessian_fn = create_hessian_function(
            calculator, calculator_type, device=device, apply_eckart=config.apply_eckart
        )
    ...
    # Track eigenvalue progress
    best_neg_eigvals = float('inf')
    steps_since_improvement = 0
    total_steps = 0
    converged = False
    early_stopped = False
    last_neg_eigvals = None

    # Run optimization with eigenvalue monitoring
    while total_steps < max_steps:
        # Run a batch of steps
        steps_to_run = min(eigval_check_interval, max_steps - total_steps)

        try:
            with SuppressStdout():
                converged = opt.run(fmax=config.fmax, steps=steps_to_run)
        except Exception:
            break
        ...
        if total_steps % eigval_check_interval == 0 or total_steps >= max_steps:
            try:
                # Get current Hessian eigenvalues
                if hasattr(opt, 'pes') and hasattr(opt.pes, 'H'):
                    H = opt.pes.H.asarray()
                    eigvals = np.linalg.eigvalsh(H)
                    current_neg = int(np.sum(eigvals < -1e-6))
                    last_neg_eigvals = current_neg

                    if current_neg < best_neg_eigvals:
                        best_neg_eigvals = current_neg
                        steps_since_improvement = 0
                        if verbose:
                            print(f"  Step {total_steps}: neg_eigvals improved to {current_neg}")
                    else:
                        steps_since_improvement += eigval_check_interval

                    # Check if we should stop early
                    if steps_since_improvement >= eigval_patience:
                        if verbose:
                            print(f"  Early stopping at step {total_steps}: no improvement in {eigval_patience} steps")
                        early_stopped = True
                        break

                    # If we've reached 1 negative eigenvalue (TS), we can stop
                    if current_neg == 1:
                        if verbose:
                            print(f"  Found TS at step {total_steps} (1 negative eigenvalue)")
                        break
            except Exception:
                pass
    ...
    # Get final state
    final_coords = torch.tensor(atoms.get_positions(), dtype=torch.float32)
    ...
    return {
        ...
        "wall_time": wall_time,
        "last_neg_eigvals": last_neg_eigvals,
        "best_neg_eigvals": best_neg_eigvals if best_neg_eigvals != float('inf') else None,
    }


def hip_sella_worker_sample_v2(predict_fn, calculator, payload, device: str) -> Dict[str, Any]:
    """Worker function for processing a single sample with eigenvalue-based pruning."""
    ...
    try:
        with SuppressStdout():
            out_dict = run_sella_with_eigval_pruning(
                calculator,
                "hip",
                start_coords,
                atomic_nums,
                ...
                max_steps,
                device,
                eigval_patience=EIGVAL_PATIENCE,
                eigval_check_interval=EIGVAL_CHECK_INTERVAL,
                verbose=False,
            )
        wall_time = time.time() - t0
    ...
    early_stopped = bool(out_dict.get("early_stopped", False))
...
def run_trial_evaluation_parallel(
    ...
    for sample_res in sample_results:
        ...
        early_stopped = bool(sample_res.get("early_stopped", False))
        if early_stopped:
            result.n_early_stopped += 1
...
def create_objective(
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        # Narrowed ranges based on hip_sella_parallel_2.db top 20 trials analysis:
        # - delta0: 0.1 to 1.5, avg 0.47 -> keep range, log scale
        # - rho_dec: 15.5 to 94.5, avg 67 -> narrow to 40-95
        # - rho_inc: 1.03 to 1.14, avg 1.10 -> keep similar
        # - sigma_dec: 0.61 to 0.96, avg 0.82 -> keep range
        # - sigma_inc: 1.05 to 1.66, avg 1.38 -> narrow to 1.1-1.7
        # - fmax: 0.0001 to 0.0145 -> keep similar
        # - apply_eckart: mostly False (75% in top 20) -> keep both options

        delta0 = trial.suggest_float("delta0", 0.1, 1.5, log=True)
        rho_dec = trial.suggest_float("rho_dec", 40.0, 95.0)  # Narrowed based on top trials
        rho_inc = trial.suggest_float("rho_inc", 1.03, 1.14)  # Narrowed
        sigma_dec = trial.suggest_float("sigma_dec", 0.6, 0.96)
        sigma_inc = trial.suggest_float("sigma_inc", 1.1, 1.7)  # Narrowed
        fmax = trial.suggest_float("fmax", 1e-4, 1.5e-2, log=True)
        ...
        if verbose:
            print(
                ...
                f"Early stopped: {result.early_stop_rate:.1%}, "
        ...
        print(
            ...
            f"early_stop_rate={result.early_stop_rate:.1%}",
            file=sys.stderr,
        )
        ...
        trial.set_user_attr("early_stop_rate", result.early_stop_rate)
        ...
        trial.set_user_attr("n_early_stopped", result.n_early_stopped)
        ...
        if use_wandb:
            log_sample(
                trial_count[0] - 1,
                {
                    ...
                    "metrics/early_stop_rate": result.early_stop_rate,
                    "metrics/score": score,
                    ...
                    "counts/n_early_stopped": result.n_early_stopped,
...
def main(argv: Optional[List[str]] = None) -> None:
    parser = argparse.ArgumentParser(
        description="HIP Sella HPO v2 - Full dataset with eigenvalue-based early stopping."
    )
    parser = add_common_args(parser)
    parser.set_defaults(calculator="hip")

    parser.add_argument("--n-trials", type=int, default=50)
    parser.add_argument("--max-steps", type=int, default=8000)  # 8k steps
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="Stop sample if neg eigenvalues don't decrease in N steps.",
    )
    parser.add_argument(
        "--eigval-check-interval",
        type=int,
        default=50,
        help="Check eigenvalues every N steps.",
    )
    parser.add_argument(
        "--util-log-every",
        type=int,
    ...
    # Update global eigenvalue parameters
    global EIGVAL_PATIENCE, EIGVAL_CHECK_INTERVAL
    EIGVAL_PATIENCE = args.eigval_patience
    EIGVAL_CHECK_INTERVAL = args.eigval_check_interval
    ...
    print(f"\n{'='*80}")
    print("HIP SELLA HPO v2 - Full Dataset with Eigenvalue-Based Early Stopping")
    print(f"{'='*80}")
    print(f"Number of trials: {args.n_trials}")
    print(f"Samples per trial: {args.max_samples}")
    print(f"Max steps per sample: {args.max_steps}")
    print(f"Eigenvalue patience: {args.eigval_patience} steps")
    print(f"Eigenvalue check interval: {args.eigval_check_interval} steps")
    ...
    study_name = args.study_name or f"hip_sella_hpo_v2_{int(time.time())}"
    db_path = Path(out_dir) / f"{study_name}.db"
    storage_url = f"sqlite:///{db_path}"

    if args.wandb:
        wandb_name = args.wandb_name or f"hip-sella-hpo-v2-{args.n_trials}trials-job{job_id}"
        init_wandb_run(
            ...
            config={
                ...
                "diag_every_n": 1,
                "order": 1,
                # Updated ranges based on hip_sella_parallel_2.db analysis
                "hpo/delta0_range": [0.1, 1.5],
                "hpo/delta0_log_scale": True,
                "hpo/rho_dec_range": [40.0, 95.0],
                "hpo/rho_inc_range": [1.03, 1.14],
                "hpo/sigma_dec_range": [0.6, 0.96],
                "hpo/sigma_inc_range": [1.1, 1.7],
                "hpo/fmax_range": [1e-4, 1.5e-2],
                "hpo/fmax_log_scale": True,
                "hpo/apply_eckart_options": [True, False],
                "hpo/pruner": "NopPruner",
                "objective/ts_rate_weight": 1.0,
                "objective/speed_weight": 0.01,
                "objective/sella_conv_weight": 0.001,
            },
            entity=args.wandb_entity,
            tags=["hpo", "sella", "hip", "optuna", "bayesian", "v2", "eigval-pruning", f"job-{job_id}"],
            run_dir=out_dir,
        )
    ...
    # Seed with configs based on best trials from hip_sella_parallel_2.db
    if n_existing == 0:
        print("\nSeeding TPE with best configs from previous HPO...")
        good_configs = [
            # Best trial from hip_sella_parallel_2.db
            {"delta0": 0.72, "rho_dec": 58.0, "rho_inc": 1.08, "sigma_dec": 0.77,
             "sigma_inc": 1.31, "fmax": 5.5e-4, "apply_eckart": False},
            # Second best
            {"delta0": 1.29, "rho_dec": 55.0, "rho_inc": 1.09, "sigma_dec": 0.61,
             "sigma_inc": 1.43, "fmax": 7.9e-3, "apply_eckart": False},
            # Good baseline
    ...
    processor = ParallelHIPProcessor(
        ...
        device=device,
        n_workers=args.n_workers,
        worker_fn=hip_sella_worker_sample_v2,
    ...
    try:
        objective = create_objective(
            ...
            sample_indices=None,  # Use all samples (no difficult-only)
        )
        ...
        results_path = Path(out_dir) / "hip_hpo_v2_results.json"
        results_data = {
            "study_name": study_name,
        ...
        best_config_path = Path(out_dir) / "hip_best_config_v2.json"
        ...
        if args.wandb:
            ...
            best_early_stop_rate = best_trial.user_attrs.get("early_stop_rate", 0)
            ...
            summary = {
                ...
                "best/early_stop_rate": best_early_stop_rate,
            ...
            log_artifact(
                ...
                description=f"HIP Sella HPO v2 Optuna study: {study_name}",
                metadata={
