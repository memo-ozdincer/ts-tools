"""HIP Sella Hyperparameter Optimization using Optuna.

This module implements Bayesian hyperparameter optimization for Sella TS search
with the HIP (ML) calculator, optimizing for transition state convergence.
...
- use_exact_hessian = True (HIP provides direct Hessian, no autograd)
...
def create_objective(
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        # Sample hyperparameters based on user's preliminary observations:
        # - BAD: delta0=0.048, rho_dec=5.0, sigma_dec=0.65
        # - Not too bad: delta0=0.2, rho_dec=20.0, sigma_dec=0.85
        # - OK: delta0=0.5, rho_dec=50.0, sigma_dec=0.9
        # Range: floor at "not too bad", go past "OK"
...
def main(argv: Optional[List[str]] = None) -> None:
    """Main entry point for HIP Sella HPO experiment."""
    parser = argparse.ArgumentParser(
        description="Bayesian HPO for Sella TS search with HIP calculator."
    )
    ...
    print(f"\n{'='*80}")
    print("HIP SELLA HYPERPARAMETER OPTIMIZATION (Optuna)")
    print(f"{'='*80}")
