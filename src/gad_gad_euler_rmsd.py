...
def align_ordered_and_get_rmsd(A, B) -> float:
    ...
    R, t = find_rigid_alignment(A, B); A_aligned = (R @ A.T).T + t
...
# --- MODIFIED: The core GAD function now has a 'mode' parameter ---
def run_gad_euler_on_batch(
    calculator: EquiformerTorchCalculator, batch: Batch, n_steps: int, dt: float, 
    mode: str = 'gad', stop_at_ts: bool = False
) -> Dict[str, Any]:
    """Runs simulation with selectable mode ('gad' or 'eigprod')."""
    assert int(batch.batch.max().item()) + 1 == 1, "Use batch_size=1."
    start_pos = batch.pos.detach().clone()
    
    trajectory = {k: [] for k in ["energy", "force_mean", "search_vec_mean", "eig_product"]}
    
    def _record_step(predictions: Dict[str, Any], search_vec: Optional[torch.Tensor]) -> Optional[float]:
        ...
        trajectory["search_vec_mean"].append(_mean_vector_magnitude(search_vec) if search_vec is not None else None)
        try:
            freq_info = analyze_frequencies_torch(predictions["hessian"], batch.pos, batch.z)
            eig_prod = _extract_eig_product(freq_info)
        except Exception: eig_prod = None
        trajectory["eig_product"].append(eig_prod)
        return eig_prod

    results = calculator.predict(batch, do_hessian=True)
    current_eig_prod = _record_step(results, search_vec=None)
    
    steps_taken = 0
    if stop_at_ts and current_eig_prod is not None and current_eig_prod < -1e-5: pass
    else:
        for step_i in range(1, n_steps + 1):
            ...
            if mode == 'gad':
                ...
                search_vec_flat = f_flat + 2.0 * torch.dot(-f_flat, v) * v
            
            elif mode == 'eigprod':
                evals, evecs = torch.linalg.eigh(hess_full)
                lambda0, lambda1 = evals[0], evals[1]
                v0, v1 = evecs[:, 0], evecs[:, 1]
                # The gradient descent direction for J = lambda0 * lambda1
                search_vec_flat = lambda1 * v0 + lambda0 * v1
            
            else:
                raise ValueError(f"Unknown mode: {mode}")

            search_vec = search_vec_flat.view(batch.pos.shape[0], 3).to(batch.pos.dtype)

            # Use negative for eigprod (descent), positive for GAD (ascent-like)
            step_sign = -1.0 if mode == 'eigprod' else 1.0
            batch.pos = batch.pos + step_sign * dt * search_vec

            results = calculator.predict(batch, do_hessian=True)
            current_eig_prod = _record_step(results, search_vec)
    ...
    except Exception: neg_eigvals_end = -1

    try:
        res_start = calculator.predict(batch.clone().to(pos=start_pos), do_hessian=True)
        fi_start = analyze_frequencies_torch(res_start["hessian"], start_pos, batch.z)
        neg_eigvals_start = int(fi_start.get("neg_num", -1))
    except Exception: neg_eigvals_start = -1
...
# --- (plot_trajectory needs a minor update to the label) ---
def _sanitize_formula(formula: str) -> str:
    safe = re.sub(r"[^A-Za-z0-9_.-]+", "_", formula)
    return (safe.strip("_") or "sample")

def plot_trajectory(trajectory: Dict[str, List[Optional[float]]], sample_index: int, formula: str, mode:str, out_dir: str) -> str:
    ...
    fig.suptitle(f"Trajectory for Sample {sample_index}: {formula} (Mode: {mode.upper()})", fontsize=14)
    
    axes[0].plot(timesteps, _nanify(trajectory["energy"]), marker=".", lw=1.2); axes[0].set_ylabel("Energy (eV)"); axes[0].set_title("Energy")
    axes[1].plot(timesteps, _nanify(trajectory["force_mean"]), marker=".", color="tab:orange", lw=1.2); axes[1].set_ylabel("Mean |F|"); axes[1].set_title("Force Magnitude")

    eig_ax = axes[2]
    eig_product = _nanify(trajectory["eig_product"])
    eig_ax.plot(timesteps, eig_product, marker=".", color="tab:purple", lw=1.2, label="λ₀ * λ₁"); eig_ax.axhline(0, color='grey', linestyle='--', lw=1, zorder=1)
    if len(eig_product) > 0 and not np.isnan(eig_product[0]): eig_ax.text(0.02, 0.95, f"Start: {eig_product[0]:.4f}", transform=eig_ax.transAxes, ha='left', va='top', color='tab:purple', fontsize=9)
    if len(eig_product) > 0 and not np.isnan(eig_product[-1]): eig_ax.text(0.98, 0.95, f"End: {eig_product[-1]:.4f}", transform=eig_ax.transAxes, ha='right', va='top', color='tab:purple', fontsize=9)
    eig_ax.set_ylabel("Eigenvalue Product"); eig_ax.set_title("Product of Two Smallest Hessian Eigenvalues"); eig_ax.legend(loc='best')
    
    # Updated label
    search_vec_label = "Mean |GAD|" if mode == 'gad' else "Mean |g_EigProd|"
    axes[3].plot(timesteps, _nanify(trajectory["search_vec_mean"]), marker=".", color="tab:green", lw=1.2)
    axes[3].set_ylabel(search_vec_label); axes[3].set_xlabel("Step"); axes[3].set_title("Search Vector Magnitude")
    
    fig.tight_layout(rect=[0, 0, 1, 0.96])
    filename = f"traj_{sample_index:03d}_{_sanitize_formula(formula)}_{mode}.png"
