#!/usr/bin/env python
"""Parallel SCINE runner for plain GAD baselines (no kicking) [HPO copy].
...
from src.logging.trajectory_logger_hpo import TrajectoryLogger
...
def run_gad_baseline(
    ...
    *,
    ...
    last_final_morse_index: Optional[int] = None
    ...
    for step in range(n_steps):
        ...
        last_final_morse_index = (
            int((evals_vib < -float(tr_threshold)).sum().item())
            if int(evals_vib.numel()) > 0
            else 0
        )
        ...
        if stop_at_ts and np.isfinite(eig_product) and eig_product < -abs(ts_eps):
            final_morse_index = last_final_morse_index if last_final_morse_index is not None else -1
            result = {
                "converged": True,
                "converged_step": step,
