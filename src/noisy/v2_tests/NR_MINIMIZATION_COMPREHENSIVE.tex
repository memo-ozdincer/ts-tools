...
A Comprehensive Record of Algorithm Development, Failure Analysis, and Experimental Results (v1--v8 + PIC-ARC)}
...
\subsection*{Best convergence rates (287--300 samples, 10k steps)}
...
\midrule
Strict ($n_\text{neg}=0$) & \textbf{79\%} & \textbf{59\%} & \textbf{41\%} & \textbf{30\%} \\
At $\eval \geq -0.01$ & 91\% & 90\% & 88\% & 93\% \\
\bottomrule
\end{tabular}
\end{center}

The strict rate drops sharply with noise, but relaxed convergence at $\eval_\text{min} \geq -0.01$ is \textbf{nearly flat at $\sim$90\% across all noise levels}. The performance cliff is a criterion failure, not an optimizer failure.
...
\textbf{Result:} Crossover uniformly hurts. Every $\mu_\text{max} > 0$ configuration converges fewer samples than the pure shifted Newton baseline ($\mu_\text{max} = 0$) at every noise level. The gradient direction is near-orthogonal to truth (dot product 0.03--0.11), so blending toward gradient descent dilutes the (imperfect but better-than-random) Newton direction without benefit.

\subsection{v9: Relaxed Convergence (The ``Silver Bullet'')}

Hypothesis: The dominant bottleneck is not the optimizer---it is the convergence criterion. v8 cascade data shows $>87\%$ of trajectories reach $\lambda_\text{min} \geq -0.01$ at ALL noise levels. The drop from 79\% (0.5\,$\angstrom$) to 20\% (2.0\,$\angstrom$) is almost entirely false rejections by the strict $n_\text{neg} = 0$ criterion.

\textbf{Supporting evidence:}
\begin{itemize}
  \item Only $\sim$10\% of DFT reactant minima have $n_\text{neg} = 0$ under DFTB0. The strict criterion is fundamentally inappropriate for cross-surface evaluation.
  \item The false-rejection gap (strict vs.\ $\lambda_\text{min} \geq -0.01$) grows from 13.7 points at 0.5\,$\angstrom$ to 73.0 points at 2.0\,$\angstrom$.
  \item Relaxing the criterion requires zero algorithmic changes---the optimizer runs identically.
\end{itemize}

\textbf{Design:} Use the proven best algorithmic configuration (shifted Newton $\varepsilon = 10^{-3}$ + trust region), and test 4 convergence criteria:
\begin{enumerate}
  \item \textbf{Strict:} $n_\text{neg} = 0$ (baseline).
  \item \textbf{Tight relaxed:} $\lambda_\text{min} \geq -0.001$ AND force $< 10^{-4}$.
  \item \textbf{Moderate relaxed:} $\lambda_\text{min} \geq -0.005$ AND force $< 10^{-4}$.
  \item \textbf{Standard relaxed:} $\lambda_\text{min} \geq -0.01$ AND force $< 10^{-4}$.
\end{enumerate}
Grid: 4 thresholds $\times$ 4 noise levels = 16 combos $\times$ 300 samples.

\textbf{Expected result:} Based on v8 cascade data, standard relaxed ($\lambda_\text{min} \geq -0.01$) should achieve $>87\%$ convergence at ALL noise levels, effectively eliminating the performance cliff.
...
\subsection{v8 Results (287 samples, 0.5--2.0\,$\angstrom$ noise)}

\begin{table}[H]
\centering
\caption{v8 results: pure shifted Newton + line search (cm=0 baseline) across all noise levels.}
\label{tab:v8-baseline}
\begin{tabular}{rcccc}
\toprule
Noise & Conv & Rate & Steps & Wall \\
\midrule
$0.5\;\angstrom$ & 209/287 & \textbf{72.8\%} & 427 & 126s \\
$1.0\;\angstrom$ & 144/287 & 50.2\% & 795 & 269s \\
$1.5\;\angstrom$ & 88/287 & 30.7\% & 1409 & 385s \\
$2.0\;\angstrom$ & 56/287 & 19.5\% & --- & --- \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Crossover uniformly hurts:} At every noise level, the pure shifted Newton baseline (cm=0) outperforms all crossover configurations. The main-effect analysis:

\begin{center}
\begin{tabular}{lc}
\toprule
$\mu_\text{max}$ & Mean convergence rate \\
\midrule
0 (pure SN) & \textbf{43.3\%} \\
0.1 & 26.9\% \\
0.5 & 17.4\% \\
1.0 & 18.3\% \\
2.0 & 15.9\% \\
...
The hypothesis that gradient descent would help at high noise was validated in principle---SD direction is better than Newton in 70\% of cases at 2.0\,$\angstrom$---but both directions are nearly orthogonal to truth ($< 0.12$ dot product), so damping curvature information only discards useful signal without providing a better alternative.

\textbf{The critical insight is in the cascade table:}

\begin{table}[H]
\centering
\caption{v8 cascade analysis for SN + LS cm=0 (strict vs relaxed convergence). The false-rejection gap is 18--73 percentage points.}
\label{tab:v8-cascade}
\begin{tabular}{rccccc}
\toprule
Noise & Strict ($n_\text{neg}=0$) & $\eval \geq -10^{-3}$ & $\eval \geq -5 \times 10^{-3}$ & $\eval \geq -10^{-2}$ & \textbf{Gap} \\
\midrule
$0.5\;\angstrom$ & 72.8\% & 78.7\% & 87.5\% & \textbf{90.9\%} & 18 pts \\
$1.0\;\angstrom$ & 50.2\% & 61.3\% & 79.8\% & \textbf{90.2\%} & 40 pts \\
$1.5\;\angstrom$ & 30.7\% & 45.6\% & 74.9\% & \textbf{87.5\%} & 57 pts \\
$2.0\;\angstrom$ & 19.5\% & 48.1\% & 81.2\% & \textbf{92.7\%} & 73 pts \\
\bottomrule
\end{tabular}
\end{table}

\textbf{The optimizer already succeeds for $>$87\% of molecules at every noise level.} The ``failure'' is almost entirely the strict $n_\text{neg} = 0$ criterion rejecting geometries with residual tiny negative eigenvalues $> -0.01$. Combined with the diagnostic from labelled minima (Section~\ref{sec:diagnostic-results}) showing that only 10\% of DFT minima have $n_\text{neg} = 0$ under DFTB0, this makes the strict criterion fundamentally inappropriate.

\subsection{PIC-ARC Results (287 samples, 0.5--2.0\,$\angstrom$ noise)}
\label{sec:pic-arc}

A clean-sheet two-phase optimizer (PIC-ARC) was designed and tested:
\begin{itemize}[nosep]
  \item \textbf{Phase A (FLOW):} Preconditioned steepest descent (Lindh-type model Hessian as PD metric) with trust-region acceptance.
  \item \textbf{Phase B (ARC):} Subspace adaptive cubic regularization in a low-dimensional subspace of $\{M^{-1}\vect{g}, \evec_1^{-}, \dots, \evec_r^{-}\}$.
  \item Conservative FLOW$\to$ARC state machine, multi-channel termination (STRICT/RELAXED/STALLED).
\end{itemize}

\textbf{Result: 0\% convergence across all 24 configurations and all noise levels.}

PIC-ARC failed completely because the FLOW phase is fundamentally a first-order method. The critique of the proposal predicted this: gradient direction has dot product $< 0.12$ with truth, so even chemistry-aware preconditioning produces nearly random steps. The trust region limited damage but could not overcome the near-orthogonality of the descent direction. The ARC phase requires stable conditions (low $\kappa$, few negative modes, low forces) which FLOW never achieved in 10,000 steps.

\textbf{Lesson:} The shifted Newton Hessian information is not optional---it is the only reason the optimizer makes meaningful progress at all. First-order methods cannot navigate this landscape.
...
\begin{table}[H]
\centering
\caption{Best convergence rate at each noise level across all algorithm versions (287--300 samples).}
\label{tab:noise-summary}
\begin{tabular}{lcclc}
\toprule
Noise & Strict & Config & Version & At $\eval \geq -0.01$ \\
\midrule
$0.5\;\angstrom$ & \textbf{79\%} & SN $\varepsilon=10^{-3}$ + TR & v6 & 91\% \\
$1.0\;\angstrom$ & \textbf{59\%} & SN $\varepsilon=10^{-3}$ + TR & v7 & 90\% \\
$1.5\;\angstrom$ & \textbf{41\%} & SN $\varepsilon=10^{-3}$ + TR & v7 & 88\% \\
$2.0\;\angstrom$ & \textbf{30\%} & SN $\varepsilon=10^{-3}$ + TR & v7 & 93\% \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key observation:} The strict ($n_\text{neg} = 0$) convergence rate drops sharply with noise, but the relaxed rate at $\eval \geq -0.01$ is \textbf{nearly flat at $\sim$90\% across all noise levels}. The performance cliff between 1.0 and 1.5\,$\angstrom$ is a \emph{criterion failure}, not an optimizer failure. The cascade data (Table~\ref{tab:v8-cascade}) demonstrates that shifting the acceptance threshold from $n_\text{neg} = 0$ to $\eval_\text{min} \geq -0.01$ would immediately raise convergence to $>$87\% at all noise levels---the ``silver bullet'' that no algorithmic elaboration has achieved.
...
\begin{enumerate}
  ...
  \item \textbf{iHiSD crossover (v8):} Crossover uniformly hurts at all noise levels. Pure SN (cm=0) mean rate 43.3\% vs cm=0.5 at 17.4\%. The GD direction is only marginally better than Newton but both are near-orthogonal to truth, so damping away curvature discards useful information without providing a viable alternative.
  \item \textbf{PIC-ARC (clean-sheet optimizer):} 0\% convergence across all 24 configurations. Preconditioned gradient descent (FLOW phase) cannot navigate the PES even with a molecule-aware metric. First-order methods fail catastrophically---the Hessian information in shifted Newton is essential, not optional.
...
\begin{enumerate}
  \item \textbf{Trust radius collapse:} Remains unsolved. Line search is worse, not better. PIC-ARC's alternative (preconditioned GD) is catastrophically worse.
  ...
  \item \textbf{Direction quality at high noise:} Both SD and Newton directions are nearly orthogonal to the true direction (dot product $< 0.12$). PIC-ARC and v8 crossover both confirmed: first-order alternatives are worse, not better. The Hessian information is essential.
  ...
  \item \textbf{Persistently hard samples:} Samples 000, 067, 134, 172, 180, 183 achieve 0\% convergence across all 24 v8 configs.
  \item \textbf{False-rejection gap is the dominant bottleneck (NEW):} The cascade analysis (Table~\ref{tab:v8-cascade}) reveals that $>$87\% of trajectories reach $\eval_\text{min} \geq -0.01$ at all noise levels. The strict-vs-relaxed gap is 18--73 percentage points. \textbf{Relaxed convergence ($\eval_\text{min} \geq -0.01$) is the single highest-impact remaining intervention}---it would raise convergence from 20--79\% (strict) to $\sim$88--93\% (relaxed) with zero algorithmic changes.
\end{enumerate}
...
\begin{itemize}
  \item \texttt{src/noisy/v2\_tests/baselines/minimization.py} ($\sim$2250 lines): \texttt{run\_newton\_raphson}, all step builders, GDIIS, diagnostics, v9 relaxed convergence.
  \item \texttt{src/noisy/v2\_tests/baselines/pic\_arc.py} ($\sim$890 lines): PIC-ARC optimizer (0\% convergence---archived).
  \item \texttt{src/noisy/v2\_tests/runners/run\_minimization\_parallel.py} ($\sim$800 lines): CLI runner, parallel worker, result construction.
...
\begin{itemize}
  ...
  \item \texttt{minimization\_nr\_grid\_v8\_run.slurm}: 24 configs (crossover), 0.5--2.0\,$\angstrom$, 300 samples.
  \item \texttt{minimization\_pic\_arc\_v1\_run.slurm}: 24 configs (PIC-ARC), 0.5--2.0\,$\angstrom$, 300 samples.
  \item \texttt{minimization\_nr\_grid\_v9\_run.slurm}: 16 configs (relaxed convergence), 0.5--2.0\,$\angstrom$, 300 samples.
