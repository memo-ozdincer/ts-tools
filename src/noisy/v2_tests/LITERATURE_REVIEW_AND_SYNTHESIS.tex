...
findings from versions~1 through~10c of the minimization code.
...
\subsection{Best Convergence Rates (v10c)}
...
\begin{tabular}{ccccc}
\toprule
Noise & v9 GQT & v10c GQT+PLS & v10c RFO+Schl+PLS & $\Delta$ (best) \\
\midrule
0.5\,\AA & 80\% & 83\% & \textbf{89\%} & +9\,pts \\
1.0\,\AA & 60\% & 67\% & \textbf{78\%} & +18\,pts \\
1.5\,\AA & 46\% & 56\% & \textbf{63\%} & +17\,pts \\
2.0\,\AA & 29\% & \textbf{45\%} & 42\% & +16\,pts \\
\bottomrule
\end{tabular}
\caption{Best convergence rates across versions (287 samples, strict $n_\text{neg}=0$).
  RFO+Schlegel+PLS is champion at 0.5--1.5\,\AA; GQT+PLS wins at 2.0\,\AA.
  PLS contributes +7 to +18 points across all configurations.}
...
Based on all evidence, the recommended baseline is:
\begin{center}
\fbox{\textbf{RFO + Schlegel TR + PLS}}
\end{center}
With GQT+PLS as an alternative at high noise ($\geq 2.0$\,\AA).
...
\begin{enumerate}
  \item \textbf{Persistently hard samples}: In v10c, samples 184, 210, 251
    are hardest (4--5/56 configs converge).  Structural analysis needed:
    are these multi-fragment dissociation, ring-opening, or electronically
    unusual reactions?
  ...
  \item \textbf{Why does PLS help RFO so much more than GQT?}
    ...
    Testing: compare step-length distributions with/without PLS.

  \item \textbf{Population~A (blind modes) unresolved}: Almost\_converged
    and ghost\_mode failures are unaffected by any v10 intervention.
    These have $\lambda_\text{min} \in [-10^{-3}, -2\times10^{-4}]$ with
    gradient overlap $<0.1$ in the negative-eigenvalue direction.
    Possible approaches: random perturbation, mode-following, internal
    coordinate rotation.

  \item \textbf{GQT vs.\ RFO crossover at 2.0\,\AA}: GQT+PLS (45\%)
    beats RFO+Schlegel+PLS (42\%) at highest noise.  Should a
    noise-adaptive algorithm selection be considered?
  ...
  \item \textbf{SCINE SCF convergence}: Could tighter SCF thresholds reduce
    ...
    written but crashed (\texttt{NameError}: \texttt{defaultdict}).
