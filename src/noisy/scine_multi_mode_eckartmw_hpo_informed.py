...
=================================================
Narrowed search ranges based on 800-step HPO results (job 1809794).

Best trial from 800-step run (trial 14):
  - Score: -0.772 (~23% success rate at 800 steps)
  - Key findings:
    * SCINE showed OPPOSITE adaptive_delta preference vs HIP
    * adaptive_delta=False worked better (unlike HIP)
...
from src.core_algos.eckart_projection import eckart_mw_project_scine as eckart_project
...
# =============================================================================
# INFORMED BASELINE - from best 800-step trial (trial 14)
# =============================================================================
BASELINE_PARAMS = {
    # Core dynamics (from trial 14)
    "dt": 0.00213,
    "dt_max": 0.0229,
    "max_atom_disp": 0.25,

    # Plateau controller (from trial 14)
    "plateau_patience": 9,
    "plateau_boost": 2.67,
    "plateau_shrink": 0.554,

    # Escape detection (from trial 14)
    "escape_disp_threshold": 4.49e-4,
    "escape_window": 35,
    "escape_neg_vib_std": 0.622,

    # Escape perturbation (key finding: adaptive_delta=False for SCINE!)
    "escape_delta": 0.136,
    "adaptive_delta": False,  # OPPOSITE of HIP!
    "min_interatomic_dist": 0.444,
...
KNOWN_GOOD_CONFIGS = [
    {
        # Trial 14 - best from 800-step run
        "dt": 0.00213,
        "dt_max": 0.0229,
        "max_atom_disp": 0.25,
        "plateau_patience": 9,
    ...
    {
        # Variant with slight adjustments
        "dt": 0.002,
        "dt_max": 0.025,
        "max_atom_disp": 0.22,
        "plateau_patience": 10,
        ...
        "escape_window": 40,
        "escape_neg_vib_std": 0.6,
        "escape_delta": 0.15,
        "adaptive_delta": False,
        "min_interatomic_dist": 0.45,
    },
]
...
def sample_hyperparameters_informed(trial: optuna.Trial) -> Dict[str, Any]:
    ...
    Ranges are centered around trial 14's best-performing values.
    Note: SCINE prefers adaptive_delta=False (opposite of HIP)!
    """
    params = BASELINE_PARAMS.copy()
    
    # dt: narrowed from [0.0005, 0.005] to [0.0015, 0.003]
    params["dt"] = trial.suggest_float("dt", 0.0015, 0.003, log=True)
    
    # dt_max: narrowed from [0.01, 0.1] to [0.015, 0.035]
    params["dt_max"] = trial.suggest_float("dt_max", 0.015, 0.035, log=True)
    
    # max_atom_disp: narrowed from [0.1, 0.5] to [0.18, 0.32]
    params["max_atom_disp"] = trial.suggest_float("max_atom_disp", 0.18, 0.32)
    
    # plateau_patience: narrowed from [3, 20] to [6, 14]
    params["plateau_patience"] = trial.suggest_int("plateau_patience", 6, 14)
    
    # plateau_boost: narrowed from [1.2, 3.0] to [2.2, 3.0]
    params["plateau_boost"] = trial.suggest_float("plateau_boost", 2.2, 3.0)
    
    # plateau_shrink: narrowed from [0.3, 0.7] to [0.45, 0.65]
    params["plateau_shrink"] = trial.suggest_float("plateau_shrink", 0.45, 0.65)
    
    # escape_disp_threshold: narrowed from [1e-4, 1e-3] to [3e-4, 6e-4]
    params["escape_disp_threshold"] = trial.suggest_float("escape_disp_threshold", 3e-4, 6e-4, log=True)
    
    # escape_window: narrowed from [10, 50] to [28, 45]
    params["escape_window"] = trial.suggest_int("escape_window", 28, 45)
    
    # escape_neg_vib_std: narrowed from [0.2, 1.0] to [0.5, 0.8]
    params["escape_neg_vib_std"] = trial.suggest_float("escape_neg_vib_std", 0.5, 0.8)
    
    # escape_delta: narrowed from [0.05, 0.3] to [0.1, 0.2]
    params["escape_delta"] = trial.suggest_float("escape_delta", 0.1, 0.2)
    
    # FIXED: adaptive_delta=False (SCINE-specific finding!)
    params["adaptive_delta"] = False
    
    # min_interatomic_dist: narrowed from [0.3, 0.7] to [0.35, 0.55]
    params["min_interatomic_dist"] = trial.suggest_float("min_interatomic_dist", 0.35, 0.55)
...
def run_verification(predict_fn, dataloader, device, n_steps, max_samples, start_from, noise_seed):
    ...
    print("\n" + "=" * 70)
    print("VERIFICATION RUN: Using INFORMED baseline (trial 14 from 800-step)")
    print("=" * 70)
...
def main():
    parser = argparse.ArgumentParser(
        description="INFORMED Bayesian HPO for SCINE Multi-Mode Eckart-MW (4000 steps)"
    )
    ...
    # HPO configuration
    parser.add_argument("--n-trials", type=int, default=30)  # Fewer trials needed
    parser.add_argument("--n-startup-trials", type=int, default=3)
    ...
    parser.add_argument("--wandb-project", type=str, default="scine-multi-mode-hpo-informed")
    ...
    study_name = args.study_name or f"scine_multi_mode_hpo_informed_{job_id}"
    ...
    if args.wandb and WANDB_AVAILABLE:
        wandb.init(
            ...
            tags=["hpo", "scine", "multi-mode-eckartmw", "informed"],
        )
    ...
    if not args.resume and len(study.trials) == 0:
        ...
        for i, config in enumerate(KNOWN_GOOD_CONFIGS):
            ...
            print(f"    Enqueued config {i+1}: dt={config['dt']:.5f}, patience={config['plateau_patience']}")
    ...
    print(f"    Note: adaptive_delta=False fixed (SCINE-specific finding)")
