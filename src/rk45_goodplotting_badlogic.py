...
# --- RK45 Solver Class (Your working version, unchanged) ---
class RK45:
    def __init__(self, f, t0, y0, t1, rtol=1e-6, atol=1e-9, h0=None, h_max=np.inf, safety=0.9, event_fn=None, max_steps=10000):
        self.f = f; self.t = float(t0); self.t_end = float(t1); self.y = np.array(y0, dtype=float)
        self.rtol = rtol; self.atol = atol; self.h_max = h_max; self.safety = safety
        self.event_fn = event_fn; self.max_steps = max_steps; self.step_count = 0
    ...
    def step(self):
        ...
        k = np.array(k)
        y5 = y + h * np.tensordot(self.b5, k, axes=(0,0)); y4 = y + h * np.tensordot(self.b4, k, axes=(0,0))
        err_norm = np.linalg.norm((y5 - y4) / (self.atol + np.maximum(np.abs(y), np.abs(y5)) * self.rtol)) / np.sqrt(err.size) if 'err' in locals() else 0
        if err_norm <= 1.0:
            self.t, self.y, self.step_count = t + h, y5, self.step_count + 1
...
# --- Core Dynamics Function (Your working version, unchanged) ---
class GADDynamics:
    def __init__(self, calculator, atomic_nums, stop_at_ts=False, kick_enabled=False, kick_force_threshold=0.015, kick_magnitude=0.1):
        self.calculator=calculator; self.atomic_nums=torch.tensor(atomic_nums,dtype=torch.long); self.num_atoms=len(atomic_nums)
        self.stop_at_ts=stop_at_ts; self.kick_enabled=kick_enabled; self.kick_force_threshold=kick_force_threshold
        self.kick_magnitude=kick_magnitude; self.device=next(calculator.potential.parameters()).device
        self.trajectory=defaultdict(list); self.ts_found=False; self.num_kicks=0
    def __call__(self, t, y, record_stats=True):
        ...
        results = self.calculator.predict(batch, do_hessian=True); forces = results["forces"]
        hessian = _prepare_hessian(results["hessian"], self.num_atoms)
        freq_info = analyze_frequencies_torch(results["hessian"], coords, self.atomic_nums); eigvals = freq_info["eigvals"]
        ...
        if self.kick_enabled:
            ...
            if force_magnitude < self.kick_force_threshold and eigvals.numel() >= 2:
                ...
                if eig0 > 0 and eig1 > 0: apply_kick=True; self.num_kicks += 1
        if apply_kick:
            kick_direction = torch.randn_like(coords); kick_direction = kick_direction / (kick_direction.norm() + 1e-12)
            velocity = kick_direction * self.kick_magnitude
            if record_stats: print(f"  [KICK {self.num_kicks}] at t={t:.3f}: |F|={force_magnitude:.4f} eV/Å, λ₀={eig0:.6f}, λ₁={eig1:.6f}")
        else:
            v = evecs_full[:, 0].to(forces.dtype); v /= (torch.norm(v) + 1e-12); f_flat = forces.reshape(-1)
            gad_flat = f_flat + 2.0 * torch.dot(-f_flat, v) * v
            velocity = gad_flat.reshape(self.num_atoms, 3)
        if record_stats:
            self.trajectory["time"].append(t); self.trajectory["energy"].append(results["energy"].item())
            ...
            self.trajectory["eig_product"].append(eig_prod); self.trajectory["neg_num"].append(freq_info.get("neg_num", -1))
            self.trajectory["kick_applied"].append(1 if apply_kick else 0)
            if self.stop_at_ts and eig_prod is not None and eig_prod < -1e-5: self.ts_found = True
        return velocity.cpu().numpy().flatten()
    def event_function(self): return self.ts_found

# --- MODIFIED: Plotting function with new filename convention ---
def plot_trajectory(trajectory, sample_index, formula, out_dir, start_from, initial_neg_num, final_neg_num):
    ...
    eig_ax.plot(timesteps, eig_prod, marker=".", color="tab:purple", lw=1.2, label="λ₀ * λ₁"); eig_ax.axhline(0, color='grey', ls='--', lw=1)
...
# --- MODIFIED: Main script execution block with full analysis ---
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run GAD-RK45 search for transition states.")
    parser = add_common_args(parser)
    parser.add_argument("--t-end", type=float, default=2.0); parser.add_argument("--max-steps", type=int, default=10000)
    ...
    parser.add_argument("--enable-kick", action="store_true"); parser.add_argument("--kick-force-threshold", type=float, default=0.015)
    ...
    print(f"Running GAD-RK45 Search. Start: {args.start_from.upper()}, Stop at TS: {args.stop_at_ts}, Kick: {args.enable_kick}")

    results_summary = []
    # --- NEW: Data collectors for final summary ---
    ...
    for i, batch in enumerate(dataloader):
        ...
        try:
            ...
            dynamics_fn = GADDynamics(calculator, batch.z.numpy(), args.stop_at_ts, args.enable_kick, args.kick_force_threshold, args.kick_magnitude)
            solver = RK45(f=dynamics_fn, t0=0.0, y0=initial_coords.numpy().flatten(), t1=args.t_end, h_max=0.1, event_fn=dynamics_fn.event_function, max_steps=args.max_steps)
            
            dynamics_fn(0.0, initial_coords.numpy().flatten(), record_stats=True) # Record initial state
            solver.solve() # Run integration
            ...
            summary = {
                ...
                "steps_taken": len(traj["time"]) - 1, "final_time": solver.t,
                "initial_eig_product": traj["eig_product"][0], "final_eig_product": traj["eig_product"][-1],
                "initial_neg_eigvals": initial_neg_num, "final_neg_eigvals": final_neg_num,
            ...
            # --- NEW: Collect data for stalled runs analysis ---
    ...
    # --- NEW: Comprehensive Final Summary Block ---
    if results_summary:
        print("\n" + "="*60)
        ...
        # Table 2: Final State Distribution
        ...
        # Table 3: Analysis of Stalled (0 -> 0) Runs
