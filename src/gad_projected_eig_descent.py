...
# --- Core Optimization Function with HYBRID approach ---
def run_eigenvalue_descent(
    ...
    and analyzes the results using the PROJECTED Hessian.
    """
    ...
    for step in range(n_steps):
        ...
        # We need gradients for the optimization part
        with torch.enable_grad():
            _, _, out = potential.forward(batch, otf_graph=True)
            
        # --- OPTIMIZATION using the FULL, UNPROJECTED Hessian ---
        ...
        # --- ANALYSIS using the non-differentiable PROJECTED Hessian ---
        # We detach the inputs to prevent any gradient tracking attempts.
        with torch.no_grad():
        ...
        # Logging the physically meaningful projected values
        history["loss"].append(loss.item()) # Log the loss we actually optimized
        history["projected_eigval_0"].append(projected_eigvals[0].item())
        history["projected_eigval_1"].append(projected_eigvals[1].item())
        
        if step % 10 == 0:
            print(f"  Step {step:03d}: Loss(unproj)={loss.item():.4f}, Proj λ0*λ1={(projected_eigvals[0]*projected_eigvals[1]).item():.4f}")

    final_coords = coords.detach().clone()
    
    # Final projected analysis
...
# --- (__main__ block is mostly the same, just adapted for the new function) ---
if __name__ == "__main__":
    # ... (parser setup is the same) ...
        ...
        try:
            ...
            with torch.no_grad():
                initial_freq_info = analyze_frequencies_torch(
                    calculator.predict(batch, do_hessian=True)["hessian"], 
                    initial_coords, 
                    atomic_nums
                )
    ...
    out_json = os.path.join(out_dir, f"full_eig_descent_{args.start_from}_{len(results_summary)}.json")
